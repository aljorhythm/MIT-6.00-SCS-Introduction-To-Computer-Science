...
def run():
    ...
    firstName = input('Enter your date of birth:\n**')
...
num = float(input('root() Enter rootnum: '))
while not num == 0:
    print ("root: " + str(root(num)) + " math.sqrt: " + str(math.sqrt(num)))
    num = float(input('root() Enter rootnum: '))

print ("\ntesting root with error margin")
num = float(input('rootErrMar() Enter rootnum: '))
err = float(input('rootErrMar() Enter error margin: '))
while not num == 0:
